...
def plot_graph(args, train_xs, dev_xs):
    ...
    cluster_num = 7
    iterations = 10
    train_ll = np.zeros([10,6])
    dev_ll = np.zeros([10,6])

    for i in range(1, iterations+1):
        print(i)
        for c in range(5, cluster_num+1):
            args.iterations = i
            args.cluster_num = c
            model = init_model(args)
            model = train_model(model, train_xs, dev_xs, args)
            train_ll[i-1][c-5] = average_log_likelihood(model, train_xs, args)
            dev_ll[i-1][c-5] = average_log_likelihood(model, dev_xs, args)
...
def main():
    ...
    init_group.add_argument('--plot', action='store_true', default=False, help='If provided, analysis result graphs will be plotted')
